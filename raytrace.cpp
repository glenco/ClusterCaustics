...
int main(int arg,char **argv){
  ...
  GridMap gridmap(&lens,Npix,center.x,range);
  {
    ...
    for(int j = 0; j < ys.size() ; ++j ){
      ...
      image_pos[j] = lens.find_images(gridmap,ys[j],lens.getSourceZ(),gridmap.getCenter(),gridmap.getXRange(),gridmap.getResolution()/3);
    ...
    for(int j = 0; j < ys.size() ; ++j ){
      ...
      // find higher resolution image positions
      image_pos2[j] = lens.find_images(gridmap2,ys[j],lens.getSourceZ(),gridmap.getCenter(),gridmap.getXRange(),gridmap.getResolution()/3);
